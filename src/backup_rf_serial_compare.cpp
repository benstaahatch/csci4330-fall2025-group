...
private:
    ...
    vector<int> getRandomFeatures(int nFeatures)
    {
        vector<int> allFeatures(nFeatures);
        iota(allFeatures.begin(), allFeatures.end(), 0);
        shuffle(allFeatures.begin(), allFeatures.end(), rng);
...
public:
    ...
    void fit(const vector<vector<double>>& X, const vector<int>& y)
    {
        ...
        cout << "Training " << nEstimators << " trees...\n";
        ...
        for (int i = 0; i < nEstimators; ++i) {
            ...
            vector<int> y_sample;
            if (bootstrap) {
                getBootstrapSample(X, y, X_sample, y_sample);
            } else {
                X_sample = X;
                y_sample = y;
            }

            vector<int> featureIndices = getRandomFeatures(nFeatures);
    ...
    vector<int> predict(const vector<vector<double>>& X) const
    {
        vector<vector<int>> allPredictions;
        allPredictions.reserve(trees.size());
        for (const auto* tree : trees) {
            allPredictions.push_back(tree->predict(X));
        }

        vector<int> finalPredictions;
        finalPredictions.reserve(X.size());
        for (size_t i = 0; i < X.size(); ++i) {
            map<int, int> votes;
...
int main(int argc, char* argv[])
{
    ...
    cout << "============================================================\n";
    cout << "Backup Serial Random Forest With Current Validation Protocol\n";
    cout << "============================================================\n";
    ...
    vector<int> y_train;
    vector<int> y_val;
    extractSubset(X_raw, y, train_idx, X_train, y_train);
    extractSubset(X_raw, y, val_idx, X_val, y_val);

    auto medians = computeInvalidZeroMedians(X_train, static_cast<int>(X_train[0].size()));
    ...
    auto predictions = rf.predict(X_val);
    ...
    string run_dir = makeRunDirectory(experiment, "backup_serial_rf", 1, 1);
    writeMetricsJson(
        run_dir,
